    ...
    const connect = () => {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.drones) {
            // Protect simulated drones — don't let WS telemetry overwrite them
            const storeDrones = useSimStore.getState().drones
            const mergedDrones = data.drones.map((wsDrone) => {
              const storeDrone = storeDrones.find((sd) => sd.id === wsDrone.id)
              if (storeDrone?.isSimulated) {
                // Keep the simulated AI decision; update only raw telemetry
                return {
                  ...wsDrone,
                  action: storeDrone.action ?? wsDrone.action,
                  reason: storeDrone.reason ?? wsDrone.reason,
                  isSimulated: true,
                }
              }
              return wsDrone
            ...
            setLiveAiDrones(mergedDrones)
            setTelemetry(data)

            const selectedId = useSimStore.getState().selectedDrone
            const drone =
              mergedDrones.find((d) => d.id === selectedId) ?? mergedDrones[0]
            if (drone) {
              setTelemetry({
                ...data,
      ...
      ws.onclose = () => {
        ...
        console.log("[AEGIS] Disconnected");
