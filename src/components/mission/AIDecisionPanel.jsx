...
const styles = {
  panel: { flex: 1, padding: '24px', overflow: 'auto' },
  title: { fontFamily: 'Rajdhani, sans-serif', fontSize: '18px', fontWeight: 700, letterSpacing: '3px', color: '#00e5ff', margin: '0 0 8px' },
  subtitle: { fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#64748b', marginBottom: '24px' },
  grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' },
  card: (critical) => ({
    background: critical ? 'rgba(239, 68, 68, 0.08)' : '#0d1117',
    border: critical ? '1px solid rgba(239, 68, 68, 0.55)' : '1px solid rgba(0, 229, 255, 0.35)',
    borderRadius: '8px',
    padding: '16px',
    boxShadow: critical ? '0 0 18px rgba(239, 68, 68, 0.12)' : '0 0 18px rgba(0, 229, 255, 0.08)',
  }),
  cardHeader: { fontFamily: 'Rajdhani, sans-serif', fontSize: '14px', fontWeight: 700, letterSpacing: '2px', color: '#e2e8f0', marginBottom: '12px', paddingBottom: '8px', borderBottom: '1px solid #1e293b' },
  line: { margin: '6px 0', fontFamily: 'JetBrains Mono, monospace', fontSize: '12px', color: '#94a3b8' },
  action: { color: '#00e5ff', fontWeight: 600 },
  assist: { marginTop: '8px', color: '#00e5ff', fontSize: '11px', fontFamily: 'JetBrains Mono, monospace' },
  empty: { padding: '48px 24px', textAlign: 'center', color: '#64748b', fontFamily: 'JetBrains Mono, monospace', fontSize: '12px', border: '1px dashed #334155', borderRadius: '8px' },
}
            ...
            return (
              <div key={d.id} className="ai-card" style={styles.card(critical)}>
                <div style={styles.cardHeader}>
                  {label} // ID {d.id}
                  {isSimulated && (
                    <span style={{ color: '#fbbf24', marginLeft: '8px', fontSize: '10px', fontWeight: 600 }}>
                      ⚡ SIMULATED
                    </span>
                  )}
                  {critical && (
                ...
                <div style={styles.line}>Drone: {d.id}</div>
